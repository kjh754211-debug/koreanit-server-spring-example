package com.koreanit.spring.comment.error;
...
import com.koreanit.spring.comment.response.ApiResponse;
...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResponse<Void>> handleNoResourceFound(NoResourceFoundException e) {
    // 5xx만 스택트레이스 출력
    log.warn("[NOT_FOUND] origin={}", origin(e));

    return ResponseEntity
        .status(ErrorCode.NOT_FOUND_RESOURCE.getStatus())
        .body(ApiResponse.fail(ErrorCode.NOT_FOUND_RESOURCE.name(), "요청한 리소스를 찾을 수 없습니다"));
  }
